...
if __name__ == "__main__":
    ...
    # open text file
    with open("dataset_10336_8s.txt") as f:
    ...
    for edge in internals:
        e1, e2 = [int(e) for e in edge.split("->")]
        graph_neighbors += g_nearest_neighbors(G, e1, e2)

    print("qty graphs", len(graph_neighbors))

    # modify graphs to add root node
    for G in graph_neighbors:
        G.remove_edge(maxnode, maxnode - 1)
        G.add_edge(maxnode + 1, maxnode)
        G.add_edge(maxnode + 1, maxnode - 1)
        draw_graph(G)
    ...
    for i, G in enumerate(graph_neighbors):
        ...
        # split graphs for each pos in sequence
        for pos in range(len(seq_sample)):

            # update leaf node values
            for node, line in enumerate(leafs):
                newchar = G.nodes(node)["char"][pos]
                nx.set_node_attributes(G, {node: 1}, name="leaf")
                nx.set_node_attributes(G, {node: newchar}, name="char")
                nx.set_node_attributes(G, {node: 1}, name="tag")
                dct = {}
                for sym in ["A", "C", "G", "T"]:
    ...
    G.remove_edge(maxnode, maxnode - 1)
    G.remove_edge(maxnode, maxnode - 2)
    G.add_edge(maxnode - 1, maxnode - 2)

    string_edges = edge_maker(G)

    # write out result file
    with open("dataset_10335_12out.txt", "w") as f:
